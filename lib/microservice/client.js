...
function Client(name) {
  ...
  for (let name in config.transports) {
    ...
    if (!Transport) {
      this.logger.log('ERROR',
        util.format('transport %s does not exist', name));
      continue;
    }
    try {
  ...
  if (config.publisher) {
    ...
    if (!defaultPublisher) {
      this.logger.log('ERROR',
        util.format('publisher %s does not exist', config.publisher.name));
    }
  }
...
Client.prototype.connect = function*() {
  ...
  return yield co(function*() {
    ...
    for (let name in self._endpoints) {
      ...
      let loadBalancer = e.loadBalancer(e.loadBalancerConfig,
        publisher, self.logger);
      service[name] = makeServiceEndpoint(name,
        self.middleware, loadBalancer, self.logger);
    }
    return service;
  });
};
