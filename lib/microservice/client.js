...
function Client(name) {
  ...
  const config = cfg.get(util.format('client:%s', name));
  // check config
  if (!config) {
  ...
  // transport
  const transports = [];
  _.forIn(config.transports, (transportConfig, transportName) => {
  ...
  // setup endpoints
  this.$endpoints = {};
  _.forIn(config.endpoints, (endpointConfig, endpointName) => {
    // publisher
