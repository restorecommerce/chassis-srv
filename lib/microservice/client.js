...
let grpc = require('../transport/provider/grpc');
...
function Client(name) {
  ...
  let config = cfg.get(util.format('client:%s', name));
  // check config
  if (!config) {
    throw new Error(util.format('no client:%s config', name));
  }
  if (!config.endpoints || _.keys(config.endpoints).length === 0) {
    throw new Error('no endpoints configured');
  }
  if (!config.transports || _.keys(config.transports).length === 0) {
  ...
  for (let name in config.transports) {
    ...
    try {
      let provider = new transport(config.transports[name], this.logger);
  ...
  if (config.loadbalancer) {
    defaultLoadBalancer = loadBalancers[config.loadbalancer.name];
  }

  // detect global publisher
  let defaultPublisher;
  if (config.publisher) {
    defaultPublisher = publishers[config.publisher.name];
    if (!defaultPublisher) {
      this.logger.log('ERROR', util.format("publisher %s does not exist", config.publisher.name));
    }
  }
  ...
  for (let name in config.endpoints) {
    ...
    let publisherCfg = config.publisher;
    if (endpointCfg.publisher && endpointCfg.publisher.name) {
      publisher = publishers[endpointCfg.publisher.name];
      publisherCfg = endpointCfg.publisher;
    }
    if (!publisher) {
      if (!publisherCfg) {
        this.logger.log('ERROR', util.format("publisher configuration for endpoint %s does not exist", name));
        continue
      }
      this.logger.log('ERROR', util.format("publisher %s does not exist", publisherCfg.name));
      continue
    }

    // loadBalancer
    let loadBalancer = defaultLoadBalancer;
    let loadBalancerCfg = config.loadbalancer;
    if (endpointCfg.loadbalancer && endpointCfg.loadbalancer.name) {
      loadBalancer = loadBalancers[endpointCfg.loadbalancer.name];
      loadBalancerCfg = endpointCfg.loadbalancer;
    }
    if (!loadBalancer) {
      if (!loadBalancerCfg) {
        this.logger.log('ERROR', util.format("loadBalancer configuration for endpoint %s does not exist", name));
        continue
      }
      this.logger.log('ERROR', util.format('loadbalancer %s does not exist', loadBalancerCfg.name));
      continue
    }
  ...
  let e = function*(request, options) {
    ...
    for (let i = 1; i <= attempts; i++) {
      ...
      try {
        ...
        logger.log('ERROR', util.format('attempt %d/%d error: %s', i, attempts, err.stack));
        errs.push(err);
  ...
  return function*(req, options) {
    if (options && options.timeout) {
      ...
      return yield co(function*() {
        ...
        if(err.status === 408) {
          err = new Error('call timeout');
          let res = gen.throw(err);
           return res.value;
        }
        return {
          error: err,
...
Client.prototype.connect = function*() {
  ...
  return yield co(function*() {
    ...
    for (let name in self._endpoints) {
      ...
      service[name] = makeServiceEndpoint(name, self.middleware, loadBalancer, self.logger);
    }
    return service;
  });
}
...
Client.prototype.end = function*() {
  for (let i = 0; i < this._transports.length; i++) {
    let transport = this._transports[i];
    yield transport.end();
  }
}
