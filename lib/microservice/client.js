  ...
  let e = function*(req, options){
    ...
    for(let i = 1; i <= attempts; i++) {
      ...
      try {
        ...
        logger.log('ERROR', util.format('attempt %d/%d error: %s', i, attempts, e.stack));
        if (e.message === 'call timeout') {
          logger.log('DEBUG', util.format('attempt %d/%d returning with call timeout', i, attempts));
          // TODO Does not return, throwing kind of works
          return yield {error:e};
        }
        errs.push(e);
      }
    }
    return yield {error:errs};
  };
  return function*(req, options) {
    if(options && options.timeout) {
      let gen = e(req, options);
      let id = setTimeout(function(){
        gen.throw(new Error('call timeout'));
      }, options.timeout);
      let res = yield gen;
