...
function getOneOfIndex(field, oneofs) {
  if (_.isNil(oneofs)) {
    return undefined;
  }
...
function createFieldDescriptorProto(field, oneofs) {
  const fdp = {
    name: field.name,
    number: field.id,
...
function createEnumDescriptorProto(enumType) {
...
function createFileOptions(options) {
  return _.mapKeys(options, (value, key) => {
    return _.camelCase(key);
  });
}

function createDescriptorProto(message) {
...
function createFileDescriptorProto(file, ast) {
...
class ServerReflection {
   ...
   */
  * serverReflectionInfo(call, context) {
  ...
  * allExtensionNumbersOfType(path, req) {
    const t = this.builder.lookup(path);
    if (_.isNil(t)) {
      return {
        validHost: req.host, // TODO Is this correct?
