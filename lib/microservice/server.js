...
const kafka = require('../events/provider/kafka');
registerEventProvider(kafka.Name, kafka.Kafka);
const local = require('../events/provider/local');
...
function Server(config, logger) {
  ...
  if (this.$config.events) {
    try {
      ...
      this.logger.error('setupEvents', error);
      throw error;
    }
  }
  ...
  for (let i = 0; i < props.length; i++) {
    ...
    for (let j = 0; j < methodNames.length; j++) {
      ...
      if (_.isNil(methodCfg)) {
        that.logger.error(`endpoint ${name}.${methodName} does not have configuration`);
        continue;
      }
      if (!_.includes(methodCfg.transport, transportName)) {
        that.logger.error(`endpoint ${name}.${methodName} 
    ...
    if (_.size(_.functions(binding)) === 0) {
      that.logger.verbose(`service ${name} has no endpoints configured 
