...
function setupEvents(config, logger) {
  ...
  try {
    ...
    logger.log('ERROR', util.format('event provider %s error', name));
    throw e;
  }
}
  ...
  if (!config.endpoints || !config.transports) {
    if (config.events) {
      if (config.transports) {
        this.logger.log('WARNING', 'transports configurated but no endpoints are configurated, transports are disabled');
      }
      if (config.endpoints) {
        this.logger.log('WARNING', 'endpoints are configurated but no transports are configurated, endpoints are not available');
      }
      return;
    }
