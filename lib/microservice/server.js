...
function setupTransport(config, logger) {
  let transport = {};
  logger.log('DEBUG', 'available transport providers',
  ...
  if (!config.endpoints || !config.transports) {
    if (config.events) {
      if (config.transports) {
        this.logger.log('WARNING', 'missing endpoints configuration');
      }
      if (config.endpoints) {
        this.logger.log('WARNING', 'missing transport configuration');
      }
      return;
    }
