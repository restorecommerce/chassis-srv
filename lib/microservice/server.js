...
function makeEndpoint(server, endpoint, transportName, methodName, logger) {
  return function* (request, context) {
    ...
    try {
      ...
      if (err instanceof SyntaxError || err instanceof RangeError || err instanceof ReferenceError || err instanceof TypeError) {
        ...
        logger.log('DEBUG', util.format('request to method %s over transport %s error', context.method, context.transport), request, err);
      }
      throw err;
    }
  }
...
Server.prototype.bind = function*(service) {
  ...
  for (let i = 0; i < props.length; i++) {
    ...
    for (let j = 0; j < methodNames.length; j++) {
      ...
      binding[name] = makeEndpoint(self, service[name], transportName, name, self.logger);
