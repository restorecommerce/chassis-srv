...
Arango.prototype.insert = function*(collection, document) {
  let doc = ensureKey(_.clone(document));
...
Arango.prototype.findOne = function*(collection, id) {
  let c = this._db.collection(collection);
  let key = idToKey(id);
  let res = yield query(this._db, c, aql `
      ...
      RETURN node
  `);
  let doc = yield res.next();
  if (doc) {
    return sanitizeFields(doc);
  }
  return null;
}
