...
function buildFilter(filter, q, op, path) {
  if (!path) {
    path = 'node';
  }
  _.forEach(filter, function(value, key) {
    ...
    switch (key) {
      case '$or':
        q = buildFilter(value, q, 'or', kp);
        break;
      case '$and':
        q = buildFilter(value, q, 'and', kp);
        break;
      default:
        q = q.eq(autoCastKey(kp, value), autoCastValue(value));
    }
  });
  return q;
}
...
Arango.prototype.find = function*(collection, filter) {
  ...
  let c = this._db.collection(collection);
  if (_.size(filter) === 0) {
    let docs = yield query(this._db, c, aql `
      FOR node in ${c}
        RETURN node
      `);
  ...
  let q = qb.for('node').in('@@collection').filter(f).return('node');
  let bindVars = {
    '@collection': collection,
  };
  let res = yield queryNew(this._db, q, bindVars);
...
Arango.prototype.delete = function*(collection, filter) {
  ...
  let f = buildFilter(filter, qb, 'and');
  let q = qb.for('node').in('@@collection').filter(f).remove('node').in('@@collection');
  let bindVars = {
    '@collection': collection,
  };
  yield queryNew(this._db, q, bindVars);
}
