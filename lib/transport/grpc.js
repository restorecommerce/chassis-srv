...
function Server(config, logger) {
  ...
  // check config
  if (!_.has(config, 'proto')) {
    throw new Error('server is missing proto field');
  }
  if (!_.has(config, 'package')) {
    throw new Error('server is missing package field');
  }
  if (!_.has(config, 'service')) {
    throw new Error('server is missing service field');
  }
  this._config = config;
  this._logger = logger;
  this._server = new grpc.Server();
  let protoFileName = path.join(process.cwd(), this._config.proto);
  if (!fs.statSync(protoFileName).isFile()) {
    throw new Error(util.format('Can not load protobuf file %s', protoFileName));
  }
  this._logger.log('VERBOSE', util.format('loading protobuf file %s', protoFileName));
  this._proto = grpc.load(protoFileName);
  let pkg = _.get(this._proto, this._config.package);
  this._service = pkg[this._config.service].service;
  this.name = NAME;
}
...
Server.prototype.bind = function*(service) {
  ...
  if (_.has(this._config, 'credentials.ssl')) {
    credentials = grpc.credentials.createSsl(this._config.credentials.ssl.certs);
  }
...
function Client(config, logger) {
  this.name = NAME;
  this._config = config;
  let protoFileName = path.join(process.cwd(), config.proto);
  logger.log('VERBOSE', util.format('loading protobuf file %s', protoFileName));
  this._proto = grpc.load(protoFileName);
  logger.log('VERBOSE', util.format('creating protobuf pkg %s', this._config.package));
  let pkg = _.get(this._proto, this._config.package);
  this._service = pkg[this._config.service];
}
