...
function buildProtobuf(files, root, logger) {
  ...
  if (root) {
    _.forEach(files, function(fileName, key){
      logger.log('VERBOSE', util.format('transport provider gRPC: loading protobuf file %s with root %s', fileName, root));
      ProtoBuf.loadProtoFile({file:fileName, root: root}, builder);
    });
  } else {
    _.forEach(files, function(fileName, key){
      logger.log('VERBOSE', util.format('transport provider gRPC: loading protobuf file %s', fileName));
