...
function makeNormalServerCall(client, methodName) {
  if (_.isNil(client)) {
  ...
  return function* normalServerCall(request, context) {
    const options = {};
 ...
 */
function wrapClientEndpoint(client, methodName) {
  if (!client[methodName]) {
    throw new Error(util.format('conn has no method %s', methodName));
  }
  return makeNormalServerCall(client, methodName);
}
 ...
 */
function wrapServerEndpoint(endpoint, logger) {
  return function wrap(call, callback) {
...
Server.prototype.bind = function* bind(service) {
  ...
  for (let i = 0; i < funcs.length; i++) {
    const name = funcs[i];
    binding[name] = wrapServerEndpoint(service[name], this.$logger);
  }
...
Client.prototype.makeEndpoint = function* makeEndpoint(method, instance) {
  ...
  const e = wrapClientEndpoint(conn, method);
