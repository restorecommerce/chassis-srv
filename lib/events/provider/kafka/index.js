...
class Topic {
  ...
  *$offset() {
    return yield this.$consumer.fetchOffset({ topic: this.name });
  }

  // HACK: confirm that commitOffset should be used like this
  *$reset(offset) {
    if (!this.$subscribed) {
...
class Kafka {
  ...
  constructor(config, logger) {
    ...
    this.$logger.verbose(
      util.format('Kafka loading protobuf files from root %s', protoRoot), files);
