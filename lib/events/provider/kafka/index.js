    ...
    handler: function(messageSet, topic, partition) {
      return Promise.each(messageSet, function(m) {
        return co(function*() {
          ...
          self._provider._logger.log('DEBUG',
          ...
          self._provider._logger.log('ERROR',
...
function Kafka(config, logger) {
  ...
  let timestamp = this._builder.lookup('google.protobuf.Timestamp');
  if (timestamp) {
  ...
  let messageEventPath = this.config.message ||
