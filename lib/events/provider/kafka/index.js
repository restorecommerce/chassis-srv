...
/**
 * Build protobuf definitions from files.
 * @param {array.string} files A list of protobuf files.
 * @param {string} root Root directory for locating protobuf files from.
 * @param {Logger} logger
...
/**
 * Find messages which contain event options.
 * @param {protobuf.Element} node
 * @return {array.protobuf.Message} List of messages containing event options.
 */
...
/**
 * Get event messages from builder.
...
/**
 * Create map of event messages.
 * @param {protobuf.Builder} builder
 * @param {array.protobuf.Message} List of messages containing event options
 * @param {Logger} logger
  ...
  * $send(topicName, eventName, message) {
    ...
    try {
      ...
      for (let i = 0; i < messages.length; i++) {
        const payload = new Message(messages[i]);
        const any = new Any();
        any.typeUrl = name;
      ...
      return yield this.$producer.send(values);
    } catch (err) {
