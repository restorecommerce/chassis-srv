      ...
      return co(function* parseEvent() {
        ...
        eventTopic.$provider.$logger.debug(
          util.format('kafka commit topic %s with offset %d',
            topic, m.offset));
        const waitQueue = eventTopic.waitQueue;
        waitQueue.arr = _.filter(eventTopic.waitQueue.arr, (w) => {
...
function shutdown(provider) {
  return () => {
    provider.$logger.info('Kafka signal SIGINT received');
    co(function* end() {
      yield provider.end();
    }).catch((err) => {
...
class Kafka {
  ...
  constructor(config, logger) {
    ...
    process.on('SIGTERM', shutdown(this));
  }
  ...
  * topic(topicName) {
   ...
   */
  * end() {
    const ends = [this.$producer.end()];
    _.forIn(this.$topics, (topic, key) => {
      if (topic.$consumer) {
        ends.push(topic.$consumer.end());
      }
    });
    yield ends;
  }
