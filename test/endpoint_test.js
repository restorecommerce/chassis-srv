...
describe('endpoint.chain', function() {
  ...
  it('should return a generator function which yields an endpoint (generator)', function*() {
    assert(isGeneratorFn(middleware));
    e = yield middleware(endpoint);
    assert(isGeneratorFn(e));
  });
  it('should call each middleware in sequence from first to last when calling the endpoint', function*() {
    let result = yield e({}, {chain:[]});
